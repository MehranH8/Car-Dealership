...
import Blogdetailcard from "../../UI/Blogdetailcard/Blogdetailcard";
...
const Blogdetail = () => {
    ...
    const choosenindex = blogs.data.findIndex((id) => {
        if (id[4] == productid) {
            return blogs
        }
    })
    ...
    return (
        <div>
            <Blogdetailcard img={blogs.data[choosenindex][0]} title={blogs.data[choosenindex][1]} describ={blogs.data[choosenindex][2]} date={blogs.data[choosenindex][3]} />
        </div>
    )
}
